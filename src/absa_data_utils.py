...
class AscProcessor(DataProcessor):
    ...
    def get_test_examples(self, data_dir, fn="test.json", evalset='full'):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, fn)), "test", evalset)

    def get_labels(self):
        """See base class."""
        return ["positive", "negative", "neutral"]
    
    def _create_examples(self, lines, set_type, evalset='full'):
        """Creates examples for the training and dev sets."""
        examples = []
        for (i, ids) in enumerate(lines):
            if evalset in lines[ids] and not lines[ids][evalset]:
                continue
            guid = "%s-%s" % (set_type, ids )
