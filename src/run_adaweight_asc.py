...
def evalutate_on_train(model, eval_dataloader):
    model.eval()
    all_y_preds=[]
...
def train(args):
    ...
    all_sample_weights=torch.ones(len(all_input_ids) )/len(all_input_ids)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        estimator_weight = np.log((1. - estimator_error) +args.factor / estimator_error - args.factor )
...
def test(args):  # Load a trained model that you have fine-tuned (we assume evaluate on cpu)    
    ...
    eval_examples = processor.get_test_examples(args.data_dir, "test_"+args.evalset+".json")
    ...
    output_eval_json = os.path.join(args.output_dir, "predictions_"+args.evalset+"_"+args.evalmode+".json") 
...
def main():    
    ...
    parser.add_argument("--factor",
                    default=0.0,
                    type=float,
                    help="factor to adjust error rate.")
