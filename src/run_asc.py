...
class BertForSequenceClassificationWithSampleWeight(BertForSequenceClassification):
...
def train(args):
    ...
    train_data = TensorDataset(all_input_ids, all_segment_ids, all_input_mask, all_label_ids, all_weights)
    ...
    if args.do_valid:
        ...
        valid_all_weights = torch.tensor([f.weight for f in valid_features], dtype=torch.float)
    ...
    model = BertForSequenceClassificationWithSampleWeight.from_pretrained(modelconfig.MODEL_ARCHIVE_MAP[args.bert_model], num_labels = len(label_list) )
    model.cuda()
    # Prepare optimizer
        ...
        if args.do_valid:
            ...
            with torch.no_grad():
                ...
                if not args.use_weight:
                    ...
                    valid_loss = losses / valid_all_weights.sum().item()
...
def test(args):  # Load a trained model that you have fine-tuned (we assume evaluate on cpu)    
    ...
    eval_examples = processor.get_test_examples(args.data_dir, args.test_file)
    eval_features = data_utils.convert_examples_to_features(eval_examples, label_list, args.max_seq_length, tokenizer, "asc", args.evalmode)
    ...
    output_eval_json = os.path.join(args.output_dir, "predictions_"+args.test_file[:-5]+"_"+args.evalmode+".json") 
...
def main():    
    ...
    parser.add_argument("--use_weight",
                    default=0,
                    type=int,
                    help="Whether to use example weights during training.")
