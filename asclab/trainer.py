        ...
        for epoch in range(args.num_train_epochs):
            ...
            with torch.no_grad():
                ...
                for step, batch in enumerate(valid_dataloader):
                    batch = tuple(t.cuda() for t in batch)
                    input_ids, segment_ids, input_mask, _, _ = batch
                    with torch.no_grad():
                        logits = model(input_ids, segment_ids, input_mask)
                    y_preds = logits.detach().cpu().numpy().argmax(axis=-1)

                    valid_all_y_preds.append(y_preds)
                valid_all_y_preds=np.concatenate(valid_all_y_preds) 

                p_macro, r_macro, f_macro, _=sklearn.metrics.precision_recall_fscore_support(valid_all_label_ids, valid_all_y_preds, average='macro')
                f_macro = 100 * 2 * p_macro*r_macro/(p_macro+r_macro)
                logger.info("validation f1: %f", f_macro)
                valid_f1.append(f_macro)
            if f_macro > best_valid_f1:
                torch.save(model, os.path.join(args.output_dir, "model.pt") )
                best_valid_f1 = f_macro
                ...
                logger.info("# hard examples %i / %i", sum(incorrect), len(incorrect))
                all_contra = np.array([f.contra for f in train_features])
                incor_contra = np.logical_and(incorrect, all_contra)
                valid_stat["incor_contra"] = int(sum(incor_contra))
                valid_stat["incorrect"] = int(sum(incorrect))
                valid_stat["contra"] = int(sum(all_contra))
                valid_stat["incor_contra_total"] = int(len(incor_contra))
                
            model.train()

        with open(os.path.join(args.output_dir, "valid.json"), "w") as fw:
            json.dump({"valid_f1": valid_f1, "valid_stat": valid_stat}, fw)
