...
class AdaLogPWeight(Trainer):
    ...
    def _p_on_train(self, model, eval_dataloader):
        model.eval()
        all_y_preds=[]
        for step, batch in enumerate(eval_dataloader):
            batch = tuple(t.cuda() for t in batch)
            ...
            with torch.no_grad():
                ...
                probs = 1. - probs.gather(1, labels.view(-1, 1) ).squeeze(-1)
            y_preds = probs.detach().cpu().numpy()
    ...
    def epoch_weight_update(self, args, model, eval_dataloader, all_label_ids, all_sample_weights, train_features):
        ...
        all_p = self._p_on_train(model, eval_dataloader)
        incorrect = (all_y_preds != all_label_ids.numpy() )
        estimator_error = np.sum(incorrect, axis=0) / len(incorrect)
        estimator_weight = np.log(max(epsilon, (1. - estimator_error)) / max(epsilon, estimator_error))
        scale = np.exp(estimator_weight * incorrect) * all_p
        all_sample_weights.copy_(torch.from_numpy(scale).float() )
